...
void thread_pool_init(int tid, int num_threads) {
    // Reserve 50% of heap for global allocator, split rest among threads
    // This allows fallback when pools are exhausted
    size_t poolable_memory = UMEM_SIZE / 2;
    size_t chunk = poolable_memory / num_threads;
    pool_start   = global_heap + (UMEM_SIZE / 2) + (tid * chunk);
...
void *umalloc_fast(size_t size) {
    size = ALIGN(size);
    if (pool_start && pool_current + size < pool_start + pool_size) {
...
void ufree(void *ptr) {
    ...
    // Check if this pointer came from thread-local pool
    // Pool allocations don't have headers, so check bounds first
    if (pool_start && (char*)ptr >= pool_start && 
        (char*)ptr < pool_current) {
        // This is from our thread pool - don't free (arena-based)
        return;
    }
    
    // Check if it's from ANY thread pool by checking global heap bounds
    if (global_heap && (char*)ptr >= global_heap && 
        (char*)ptr < global_heap + UMEM_SIZE) {
        // It's in the heap region - check if it has a valid header
        header_t *hdr = (header_t *)((char *)ptr - sizeof(header_t));
        if (hdr->magic == MAGIC) {
            // Valid header - this is a global allocation, free it
            if (use_multiprocess) {
                atomic_fetch_add(&lock_count, 1);
                pthread_mutex_lock(&mLock);
            }
        ...
        // else: no valid header = thread pool allocation from another thread, skip
        return;
    }
    
    // Not from our allocator at all - shouldn't happen
}
...
int main(int argc, char *argv[]) {
    ...
    if (use_multithread)
        use_multiprocess = 1;   // reuse the same flag for the mutex
...
int run_single(const char *filename) {
    ...
    printf("Total allocator lock acquisitions: %d\n", atomic_load(&lock_count));
    return 0;
}
...
int run_multi(const char *filename) {
    ...
    unsigned long final_hash = 0;
    int block_num = 0;
    int max_concurrent = 8;  // Limit concurrent processes

    /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     * Process blocks in batches to avoid "too many open files"
     */
    ...
    while (!feof(fp)) {
        ...
        // Read and fork up to 8 blocks
        for (int i = 0; i < max_concurrent && !feof(fp); i++) {
            size_t n = fread(buf, 1, BLOCK_SIZE, fp);
            if (n == 0) break;
        ...
        for (int i = 0; i < batch_size; i++) {
            ...
            close(pipe_fds[i]);
            print_intermediate(block_num - batch_size + i, h, pids[i]);
            final_hash = (final_hash + h) % LARGE_PRIME;
...
typedef struct {
    int block_id;
    int thread_id;  // Which thread in the pool (0-7)
...
int run_threads(const char* filename) {
    ...
    unsigned char buf[BLOCK_SIZE];
    thread_arg_t targs[8];         // Only need args for 8 concurrent threads
    ...
    /* ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     * Process blocks in batches with fixed thread pool
     * 
     * Read and allocate blocks on-demand as threads become available.
     * This avoids pre-allocating all blocks at once (memory exhaustion).
     */

    while (!feof(fp)) {
        int batch_size = 0;
        
        // Read up to num_threads blocks for this batch
        for (int i = 0; i < num_threads && !feof(fp); i++) {
            size_t n = fread(buf, 1, BLOCK_SIZE, fp);
            if (n == 0) break;
            ...
            targs[i].block_id = block_num;
            targs[i].thread_id = i;
            targs[i].block_buf = block_buf;
            targs[i].block_len = n;
        ...
        // This frees memory from completed blocks before allocating new ones
        for(int i = 0; i < batch_size; i++) {
    ...
    // Compute final hash from all results
