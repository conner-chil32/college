...
public class VerticalStrategy extends Strategy {
	Student s;
	public VerticalStrategy(Student s) {
	...
	public void move() {
		...
		if(s.getTimeRemain() > -1.0) {
			...
			if(s.checkBounds()) {
				...
				s.getLocalTransform().setTranslation((float)s.getLocalTransform().getTranslateX(), (float)Math.ceil(s.getLocalTransform().getTranslateY() + Math.sin(90.0 - Math.toRadians(s.getHead())) * s.getSpeed()));
			}
		}
