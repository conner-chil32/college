...
public class GameObject
{
	...
	*/
	...
	/** Will add the inputted yaw angle amount to the existing world rotation of the GameObject */
	public void yaw(float angle) {
		Matrix4f oldRotation = getWorldRotation();
		Vector3fc oldUp = getWorldUpVector();
		Matrix4f addedRotation = (new Matrix4f().rotation(angle, oldUp));
		Matrix4f newRotation = addedRotation.mul(oldRotation);
		setLocalRotation(newRotation);
	}

	/** Will add the inputted pitch angle amount to the existing world rotation of the GameObject */
	public void pitch(float angle) {
		Matrix4f oldRotation = getWorldRotation();
		Vector3fc oldRight = getWorldRightVector();
		Matrix4f addedRotation = (new Matrix4f().rotation(angle, oldRight));
		Matrix4f newRotation = addedRotation.mul(oldRotation);
		setLocalRotation(newRotation);
	}
